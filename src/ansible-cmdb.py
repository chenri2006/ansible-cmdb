    ...
    for col in cust_cols:
        for required_key in required_keys:
            if required_key not in col:
                sys.stderr.write(
                    "Missing required key '{}' in custom "
                    "column {}\n".format(required_key, col)
                )
                sys.exit(1)
            if "jsonxs" not in col and "tpl" not in col:
                sys.stderr.write(
                    "You need to specify 'jsonxs' or 'tpl' "
                    "for custom column {}\n".format(col)
                )
                sys.exit(1)
...
def parse_user_params(user_params):
    ...
    if user_params:
        ...
        try:
            ...
            sys.stdout.write(
                "Invalid params specified. Should be in format: <key=value>[,<key=value>..]\n"
            )
            sys.exit(1)
        return params
    else:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        action="store",
        default="html_fancy",
        help="Template to use. Default is 'html_fancy'",
    )
    parser.add_option(
        "-i",
        "--inventory",
        ...
        action="store",
        default=None,
        help="Inventory to read extra info from",
    )
    parser.add_option(
        "-f",
        "--fact-cache",
        ...
        action="store_true",
        default=False,
        help="<dir> contains fact-cache files",
    )
    parser.add_option(
        "-p",
        "--params",
        ...
        action="store",
        default=None,
        help="Params to send to template",
    )
    parser.add_option(
        "-d",
        "--debug",
    ...
    parser.add_option(
        ...
        action="store",
        default=None,
        help="Path to a custom columns definition file",
    )
    parser.add_option(
        "-l",
        "--limit",
    ...
    ansible = ansiblecmdb.Ansible(
        args, hosts_files, options.fact_cache, limit=options.limit, debug=options.debug
    )
    ...
    if renderer.tpl_file is None:
        sys.stderr.write(
            "Template '{0}' not found at any of \n  {1}\n".format(
    ...
    try:
        ...
        debug_txt = (
            ...
            "The output is probably not correct.\n\n".format(
                err.__class__.__name__, err, full_err
            )
        )
        ...
        if not options.debug:
            debug_txt += (
                "You can re-run ansible-cmdb with the -d switch to turn on debugging\n"
