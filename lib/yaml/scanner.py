...
class Scanner(object):
    ...
    def fetch_more_tokens(self):
        ...
        raise ScannerError(
            ...
            None,
            "found character %r that cannot start any token" % ch.encode("utf-8"),
            self.get_mark(),
        )
    ...
    def scan_yaml_directive_value(self, start_mark):
        ...
        if self.peek() != ".":
            raise ScannerError(
                "while scanning a directive",
                start_mark,
                "expected a digit or '.', but found %r" % self.peek().encode("utf-8"),
                self.get_mark(),
            )
        self.forward()
    ...
    def scan_directive_ignored_line(self, start_mark):
        ...
        if ch not in "\0\r\n\x85\u2028\u2029":
            raise ScannerError(
                "while scanning a directive",
                start_mark,
                "expected a comment or a line break, but found %r" % ch.encode("utf-8"),
                self.get_mark(),
            )
            ...
            if not double and ch == "'" and self.peek(1) == "'":
                ...
                if ch in self.ESCAPE_REPLACEMENTS:
                    ...
                    for k in range(length):
                        if self.peek(k) not in "0123456789ABCDEFabcdef":
                            raise ScannerError(
                                ...
                                % (length, self.peek(k).encode("utf-8")),
                                self.get_mark(),
                            )
                    ...
                    raise ScannerError(
                        "while scanning a double-quoted scalar",
                        start_mark,
                        "found unknown escape character %r" % ch.encode("utf-8"),
                        self.get_mark(),
                    )
            else:
                return chunks
            ...
            for k in range(2):
                if self.peek(k) not in "0123456789ABCDEFabcdef":
                    raise ScannerError(
                        ...
                        % (self.peek(k).encode("utf-8")),
                        self.get_mark(),
                    )
            bytes.append(chr(int(self.prefix(2), 16)))
            self.forward(2)
        try:
            value = "".join(bytes).decode("utf-8")
